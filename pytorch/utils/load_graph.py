...
def load_ogbn_dataset(name,  args):
	...
	dataset = DglNodePropPredDataset(name=name, root=os.path.join(home_dir,  "dataset"))
...
def load_karate_isolated_node():
	from dgl.data import KarateClubDataset

	# load Karate data
	# data = KarateClubDataset()
	# g = data[0]
	u=torch.tensor([0, 1, 1, 1, 1, 2, 2, 2, 3, 3, 4, 1, 5, 3, 7])
	v=torch.tensor([1, 0, 2, 3, 4, 1, 3, 4, 2, 4, 3, 5, 1, 6, 2])
	...
	for nid in range(8):
		ndata.append((th.ones(4)*nid).tolist())
	ddd = {'feat': th.tensor(ndata)}
	g.ndata['label']=torch.tensor([0,1,0,1,0,1,1, 0])
	g.ndata['feat'] = ddd['feat']
	print(g)
...
def load_ogb(name, args):
	home_dir = os.getenv("HOME")
	data = DglNodePropPredDataset(name=name, root=os.path.join(home_dir,  "dataset"))
