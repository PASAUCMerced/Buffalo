...
class Graph_Partitioner:  # ----------------------*** split the output layer block ***---------------------
	...
	def remove_non_output_nodes(self):
		...
		return to_remove # local output nodes
	...
	def simple_gen_K_batches_seeds_list(self):
		...
		if self.selection_method == "random" or self.selection_method == "range" or self.selection_method=="metis":
			...
			to_remove = self.remove_non_output_nodes() # local node ids
			to_remove = torch.tensor(to_remove)
			auxiliary_graph.remove_nodes(to_remove)
			# remove degree =0 nodes 
			nodes_degree_zero = torch.nonzero(g.in_degrees() == 0).squeeze()
			# # print('to_remove ', to_remove)
			# # print('nodes_degree_zero ', nodes_degree_zero)
			# if len(to_remove) or len(nodes_degree_zero):
			# 	if len(to_remove) and len(nodes_degree_zero):
			# 		unique_elements = torch.unique(torch.cat((to_remove, nodes_degree_zero)))
			# 	else:
			# 		unique_elements = torch.tensor(to_remove) if to_remove else torch.tensor(nodes_degree_zero)
			# 	auxiliary_graph.remove_nodes(unique_elements)

			# print('auxiliary_graph remove nodes with degree zero and non output nodes---')
			...
			for pid in partition:
				...
				overlap = self.intersection(nids, nodes_degree_zero)
				nids = nids[~torch.isin(nids, overlap)]
				res.append(sorted(nids.tolist()))
