		...
		for epoch in range(args.num_epochs):
			...
			for step, (input_nodes, seeds, blocks) in enumerate(full_batch_dataloader):
				...
				if step < args.num_batch-1:
					...
					tt += (time.time()-train_st)
					train_t = tt
				data_move_time += (train_st-epoch_start)
				loader_time += (epoch_start-before_loader_time)
			if epoch >= args.log_indent:
				...
				data_move_time_list.append(data_move_time)
				data_loader_time_list.append(loader_time)
				print('loader time/epoch {}'.format(loader_time))
				print('data move to GPU time/epoch {}'.format(data_move_time))
			...
			print()
		print('-='*40)
		print('loader time/epoch {}'.format(mean(data_loader_time_list)))
