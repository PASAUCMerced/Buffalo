	...
	def gen_batches_seeds_list(self, bkt_dst_nodes_list):
		...
		if "bucketing" in self.selection_method :
			...
			if '25_group_' in self.selection_method:
				...
				print(redundant_ratio)
				print(len(redundant_ratio))
				capacity_est = self.memory_constraint-mean(redundant_ratio)*30.85/self.args.num_batch
				print('capacity: ', capacity_est)
				...
				if len(split_batches_nid_list)>=len(G_BUCKET_ID_list):
					for j in range(len(split_batches_nid_list)):
						if G_BUCKET_ID_list[j]:
							tensor_group = torch.tensor(G_BUCKET_ID_list[j], dtype=torch.long)
				...
				# the local nid is not correct
				batches_nid_list =  split_batches_nid_list 
				self.weights_list = weights_list
				self.local_batched_seeds_list = batches_nid_list
