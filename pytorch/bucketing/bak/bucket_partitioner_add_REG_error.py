...
class Bucket_Partitioner:  # ----------------------*** split the output layer block ***---------------------
	...
	def get_in_degree_bucketing(self):
		...
		for deg, node_bkt in zip(unique_degs, bucketor(nodes)):
			...
			print("degree : " , deg)
			print('node_bkt local nid ', node_bkt)
			# print('len(bkt) ', len(node_bkt))
			print('self.layer_block.in_edges(node_bkt)')
			print(self.layer_block.in_edges(node_bkt))
			print()
	...
	def remove_non_output_nodes(self, partial_src_nid, local_seed_nids):
		import copy
		local_src=copy.deepcopy(partial_src_nid)
		if len(local_src) == 0 :
			return []			
		# mask_array = np.full(len(local_src),True, dtype=np.bool)
		mask_array = torch_is_in_1d(local_src, local_seed_nids)
		# eqidx = torch.nonzero(eqidx_bool, as_tuple=False).squeeze().view(-1)				
		# global_dst = torch.index_select(self.src_nids_tensor, 0, eqidx.long())
		# mask_array[eqidx] = False
		mask_array = ~mask_array
		to_remove = torch.nonzero(mask_array, as_tuple=False).squeeze().view(-1)
	...
	def gen_batches_seeds_list(self, bkt_dst_nodes_list):
		...
		if "bucketing" in self.selection_method :
			...
			if "REG" in self.selection_method and self.args.num_batch == 2:
				batches_nid_list = []
				batches_nid_list.append(fanout_dst_nids)
			if "REG" in self.selection_method and self.args.num_batch >= 3:
				print('REG partition on the long tail part or the second group ')
				
				print('REG start----................................')
				ts = time.time()
				# get_memory('---------================-----------------=============---------REG before start \n')
				# u, v =self.layer_block.edges()[0], self.layer_block.edges()[1] # local edges
				print('fanout_dst_nids ', fanout_dst_nids)
				u, v = self.layer_block.in_edges(fanout_dst_nids)

				partial_local_src = torch.unique(u)

				c=OrderedCounter(partial_local_src.tolist())
				list(map(c.__delitem__, filter(c.__contains__,fanout_dst_nids.tolist())))
				print(type(c.keys()))
				r_=list(c.keys())
				tail = torch.tensor(r_,dtype=torch.long)
				partial_local_src = torch.cat((fanout_dst_nids , tail))
				...
				# print(auxiliary_graph.dstdata['_ID'])
				# print(auxiliary_graph.srcdata['_ID'])
				to_remove = self.remove_non_output_nodes(partial_src_nid=partial_local_src ,local_seed_nids=fanout_dst_nids)
				if len(to_remove)>0:
					print(auxiliary_graph.nodes())
					print(auxiliary_graph.edata['w'])					
					auxiliary_graph.remove_nodes(to_remove)
				auxiliary_graph_no_diag = dgl.remove_self_loop(auxiliary_graph)
				# res = Counter(auxiliary_graph_no_diag.edata['w'].tolist())
				tp1 = time.time()
				partition = dgl.metis_partition(g=auxiliary_graph_no_diag,k=self.args.num_batch-1)
				tp2 = time.time()
				res=[]
				for pid in partition:
					print(partition[pid].ndata[dgl.NID]) 
					nids_REG = partition[pid].ndata[dgl.NID].tolist() # the nid of the REG
					nids = fanout_dst_nids[nids_REG]
	...
	def local_to_global(self):
		...
		for local_seed_nids in self.local_batched_seeds_list:
			...
			if 'REG' in self.selection_method:
				local_src = torch.tensor(list(range(len(self.src_nids_tensor))))
				eqidx_bool = torch_is_in_1d(local_src, local_seed_nids)
				eqidx = torch.nonzero(eqidx_bool, as_tuple=False).squeeze().view(-1)				
				global_dst = torch.index_select(self.src_nids_tensor, 0, eqidx.long())
				self.global_batched_seeds_list.append(global_dst)
				print('global dst ', global_dst)
				print()
