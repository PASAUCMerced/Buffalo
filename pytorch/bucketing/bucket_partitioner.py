...
class Bucket_Partitioner:  # ----------------------*** split the output layer block ***---------------------
	...
	def get_in_degree_bucketing(self):
		...
		print('get_in_degree_bucketing dst global nid length', len(self.layer_block.dstdata['_ID']))
		...
		# print('len(self.layer_block.dstdata[_ID]) ', len(self.layer_block.dstdata['_ID']))
	...
	def mean_aggre_mem_est(self, bkt_dst_nodes_list):
		if 'products' in self.selection_method :
	...
	def gen_batches_seeds_list(self, bkt_dst_nodes_list):
		...
		if  'random' in self.selection_method or 'range' in self.selection_method or 'metis' in self.selection_method :
			...
			if 'mean' in self.aggre:
				mean_aggre_mem_est(self, bkt_dst_nodes_list)
				return
			elif 'pool' in self.aggre:
				mean_aggre_mem_est(self, bkt_dst_nodes_list)
				return
				...
				if "_backpack_" in self.selection_method :
					...
					if "25_backpack_" in self.selection_method : # 2-layers
						...
						if self.hidden == 128:
							estimated_mem = [0.03354020171634035, 0.0789222900831052, 0.14299627167576137, 0.2318244492153784, 0.33085486366172856, 0.4555005140345204, 0.5405769855622832, 0.7047147945388569, 0.7768355458776156, 0.9281502018095925, 1.0675311485230141, 1.0285266294845892, 1.2832128918560093, 1.291814475094105, 1.367826536234784, 1.4645761989081698, 1.6565154083265854, 1.5991467489715145, 1.793769060001223, 1.6766132844613435, 1.920127067184496, 2.1946883605045633, 2.232189497508864, 2.223171411479282]
							# sum estimated_mem[:-1] = 27.002, estimated_mem[-1]=18.92
							if self.memory_constraint > 60:
								if self.num_batch == 2:
									capacity_imp = 16 # memory constraint minus split bucket generated micro-batch capacity for group mem
														# (60-20)/2 = 20
								elif self.num_batch == 4:
									capacity_imp = 9    	# nb=4, CUDA memeory cost is 47.005 GB
								elif self.num_batch == 12:
									capacity_imp = 3    	# nb=12, CUDA memeory cost is 21.21 GB
								elif self.num_batch == 16:
									capacity_imp = 2.2322    	# nb=16, CUDA memeory cost is 16.82 GB
								elif self.num_batch == 18:
									capacity_imp = 2.2322    	# nb=18, CUDA memeory cost is 15.34 GB
							elif self.memory_constraint < 20:
								if self.num_batch < 12:
									print('the number of batch '+str(self.num_batch)+' is too small, will cause OOM, we increase it')
									self.num_batch = 12  # when number of batches, the cost will OOM.
									self.K = 12
									local_split_batches_nid_list = list(torch.chunk(fanout_dst_nids, self.K))
									print("we can reschedule split K-->K+1 until break OOM")
						...
						ff = '/home/cc/Buffalo/pytorch/bucketing/fanout_est_mem/fanout_800_est_mem.txt'
					...
					for j in range(len(g_bucket_nids_list)):
						current_group_mem = get_sum(G_BUCKET_ID_list[j], estimated_mem)
						print("current group_mem ", current_group_mem)
						# local_split_batches_nid_list[j] = torch.tensor(local_split_batches_nid_list[j]) # split result
						local_split_batches_nid_list[j] = torch.cat((local_split_batches_nid_list[j], g_bucket_nids_list[j])) # split + group results
						# local_split_batches_nid_list[j]=torch.tensor(g_bucket_nids_list[j]) # group result
						# res_tmp.append(g_bucket_nids_list[j])
					time_batch_gen_end = time.time()
					print('batches output list generation spend ', time_batch_gen_end-time_batch_gen_start)
					length = len(self.output_nids)
	...
	def local_to_global(self):
		print('self.has_zero_indegree_seeds ', self.has_zero_indegree_seeds)
		# print('local_to_global: src global ', self.src_nids_tensor)
		if self.has_zero_indegree_seeds and 'metis' not in self.selection_method:
			# print('self.local_seeds_degree_zero', self.local_seeds_degree_zero)
			# print('self.local_batched_seeds_list ', self.local_batched_seeds_list)
			self.local_batched_seeds_list = self.process_tensors(self.local_batched_seeds_list,self.local_seeds_degree_zero)
			self.weights_list = self.get_weights(self.local_batched_seeds_list)
			print('after correction, the weights list ', self.weights_list)
		num_output =0
